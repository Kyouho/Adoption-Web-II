        ...
        <div style="display:grid; grid-template-columns:320px 1fr">
            ...
            <div style="padding:2rem 2.5rem 2rem 3rem; border-left:.5px solid #e5e7eb" class="flex flex-col">
                ...
                <div class="flex items-start justify-between gap-3 mb-1">
                    <h1 class="font-serif text-2xl font-semibold text-gray-900">{{ $animal->name }}</h1>
                    @if($animal->status === 'available')
                        <span class="flex-shrink-0 text-xs font-medium px-3 py-1 rounded-full bg-green-50 text-green-700 border border-green-100">
                            Disponible
                        </span>
                    @elseif($animal->status === 'in_process')
                        <span class="flex-shrink-0 text-xs font-medium px-3 py-1 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-100">
                            En proceso
                        </span>
                    @else
                        <span class="flex-shrink-0 text-xs font-medium px-3 py-1 rounded-full bg-gray-100 text-gray-500 border border-gray-200">
                            Adoptado
                        </span>
                    @endif
                </div>

                <p class="text-sm text-gray-400 mb-6">{{ $animal->type }} · {{ $animal->breed ?? 'Raza no registrada' }}</p>
